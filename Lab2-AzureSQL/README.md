...
1. Založíme a nasadíme Azure WebJob:
   1. V *Solution Exploreru* přidejte do solution nový projekt typu *Console Application* (.NET C#).
...
1. Připojíme se k Azure SQL z VS Code a připravíme schéma databáze:
	1. Ve VS Code si nainstalujte rozšíření *SQL Server (mssql)*.
	2. Otevřete paletu příkazů a spusťte *MS SQL: Connect*.
	3. Přidejte Azure SQL server, který jste právě založili. Když vás VS Code vyzve k firewall pravidlu, potvrďte přidání aktuální IP adresy.
	4. Otevřete nový `.sql` soubor nebo Query editor a spusťte následující skript pro založení tabulky EmailQueue:
		```sql
		...
		```
	5. Tohle je první ruční krok, který máš udělat teď: potvrdit připojení a založit tabulku v Azure SQL.
...
## Postup bez Visual Studia

Pokud máš jen VS Code, pokračuj takto:

1. V projektu `MyWebJob` spusť `dotnet restore` a `dotnet run`, aby ses ujistil, že aplikace lokálně čte `appsettings.json`.
2. Do Azure App Service nastav konfiguraci přes *Configuration*:
	- `ConnectionStrings__MyDatabase`
	- `MailSettings__SmtpHost`
	- `MailSettings__SmtpFrom`
	- `MailSettings__SmtpUsername`
	- `MailSettings__SmtpPassword`
3. Až to budeš mít, otevři v Azure Portalu App Service a zkontroluj, že WebJob běží v sekci *WebJobs*.
4. Další ruční krok pro tebe bude vložit testovací řádek do tabulky přes Azure SQL Query Editor. Teprve potom půjde ověřit výstup v *Logs*.
...
1. Pro vložení testovacího záznamu do DB použij Query Editor v Azure Portalu na konkrétní Azure SQL Database:
	![11 Azur S Q L Query Explorer](img/11-AzureSQL-QueryExplorer.png)
2. Za pozornost stojí volba *Logs* v App Service > WebJobs, kde si můžeš prohlížet konzolový výstup (stdout) WebJob aplikace.
